# STATUS.historico.md — ARQUIVO DE HISTÓRICO (iFujão / StudyFlow)

> Este arquivo é o **histórico completo** (cópia fiel das sessões anteriores).
> **Não ler todo a cada sessão.** O estado atual, as regras de conduta e os
> comandos de build estão em **`STATUS.md`**. Use este arquivo só como
> referência de decisões passadas (PII/RLS, mapa/GPS, busca por IA,
> patrocinadores, etc.).
...
## Atualizações (2026-08-24) — banner "AJUDE A ENCONTRAR!" pulsante no card do pet

> `tsc --noEmit` limpo. Exige **rebuild nativo** (`npx expo run:android`) para
> validar em runtime.

### Implementação (`app/(tabs)/index.tsx`)
- Novo estado animado `helpPulse = useRef(new Animated.Value(1)).current` com
  `Animated.loop` (sequência 1 → 1.25 → 1, 700ms cada, `useNativeDriver: true`).
- No `demoOverlay` (topo do card, ao lado do botão de fechar), adicionado um
  `helpFindRow` com dois corações vermelhos (`heartBig` + `heartSmall`, ambos
  com `transform: [{ scale: helpPulse }]`) e o texto **"AJUDE A ENCONTRAR!"**
  (`helpFindBold` em vermelho + `helpFindSmall` "A ENCONTRAR!" em texto normal).
- Estilos novos: `helpFind`, `helpFindRow`, `heartWrap`, `heartBig`,
  `heartSmall`, `helpFindBold`, `helpFindSmall`.

### Arquivos alterados
| Arquivo | O que mudou |
|---|---|
| `app/(tabs)/index.tsx` | banner pulsante "AJUDE A ENCONTRAR!" (corações + texto) no topo do card do pet. |

### Notas
- O banner é puramente visual (chamada de atenção para quem avistou o pet);
  não altera fluxo de contato/denúncia.
- **Bug corrigido (pulso parava ao abrir/fechar o card):** o banner ficava
  DENTRO do `Modal` do card, e a animação original (`loop`+`sequence` com
  `useNativeDriver`) não sobrevivia ao ciclo de montar/desmontar o card (o
  `pawPulse` funciona justamente por estar FORA do `Modal`). Corrigido
  isolando o banner num componente próprio **`HelpFindBanner`** que é montado
  junto com o card: ele recria o `helpPulse` (`0→1`) e o loop a cada abertura,
  e usa **`useNativeDriver: false`** (driver JS, confiável dentro de `Modal`).
  O `scale` vem de `helpPulse.interpolate([0,0.5,1]→[1,1.25,1])`. Assim pulsa
  enquanto o card está aberto e recomeça limpo a cada abertura. `styles` é
  passado via prop (o componente é irmão do `HomeScreen`, não filho, para não
  remontar a cada poll de GPS).
- Pendente de rebuild/teste no emulador/dispositivo.
